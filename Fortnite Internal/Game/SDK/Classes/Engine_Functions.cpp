...
SDK::UClass* SDK::UPrimitiveComponent::StaticClass()
{
    static class UClass* Clss = nullptr;

    if (!Clss)
        Clss = UObject::FindClassFast(std::string(skCrypt("PrimitiveComponent")));
...
bool SDK::UKismetSystemLibrary::LineTraceSingle(class UObject* WorldContextObject, struct FVector& Start, struct FVector& End, enum class ETraceTypeQuery TraceChannel, bool bTraceComplex, TArray<class AActor*>& ActorsToIgnore, enum class EDrawDebugTrace DrawDebugType, struct FHitResult& OutHit, bool bIgnoreSelf, const struct FLinearColor& TraceColor, const struct FLinearColor& TraceHitColor, float DrawTime)
{
    ...
    if (OriginalLineTraceSingle == nullptr)
        OriginalLineTraceSingle = reinterpret_cast<LineTraceSingleParams>(SDK::GetBaseAddress() + SDK::Cached::Functions::LineTraceSingle);

    SDK::FLinearColor EmptyColor = SDK::FLinearColor();

    return OriginalLineTraceSingle(WorldContextObject, Start, End, TraceChannel, bTraceComplex, ActorsToIgnore, EDrawDebugTrace::None, OutHit, bIgnoreSelf, EmptyColor, EmptyColor, 0.f);
}
...
bool SDK::IsPositionVisible(SDK::UObject* WorldContextObj, FVector& TargetPosition, SDK::AActor* ActorToIgnore, SDK::AActor* ActorToIgnore2)
{
    ...
    auto test = LI_FN(LoadLibraryA).get();
